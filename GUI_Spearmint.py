...
from utils.sweep2d import Sweep2D
...
class UImain(QtWidgets.QMainWindow):
    ...
    def __init__(self, parent=None, config_file=None):
        ...
        # It's okay to add an entry to the sweep settings like this, probably
    ...
    def load_sweep(self):
        '''Loads a 1D Sweep. In need of refactoring/replacing 

        Runs when 'load sweep' is clicked. Pretty outdated since it only supports sweep 1D.
        '''
    ...
    def on_sweep_completed(self):
        ...
        for ds_info in self.datasets:
            try:
                ...
                filename = os.path.join(directory, f"\\{ds.run_id}_{ds.exp_name}_{ds.sample_name}.csv")
