##Modified driver - edits made by Dacen Waters, dwaters@uw.edu
...
class Source2450(InstrumentChannel):
    ...
    function_modes = {
        ...
        "voltage": {
            "name": "VOLT",
            "unit": "V",
            "range_vals": Numbers(min(LIMITS), max(LIMITS))
        }
    }
    ...
    def __init__(self, parent: 'Keithley2450', name: str, proper_function: str) -> None:
        ...
        self.gate_limits = (min(self.LIMITS), max(self.LIMITS))
            
        self.add_parameter('gate_ulim',
                           vals = Numbers(min(self.LIMITS), max(self.LIMITS)),
                           get_cmd = self._get_gate_ulim,
                           set_cmd = self._set_gate_ulim,
                           unit='V'
        )
        self.add_parameter('gate_llim',
                           vals = Numbers(min(self.LIMITS), max(self.LIMITS)),
                           get_cmd = self._get_gate_llim,
                           set_cmd = self._set_gate_llim,
                           unit='V'
        )
...
#             get_cmd=f"SOUR:{self._proper_function}?",
    ...
    def _set_source(self, setpoint):
        if self._proper_function == "current":
            message = f"SOUR:{self._proper_function} " + f"{setpoint}"
            self.write_raw(message)
        elif setpoint > self.gate_ulim():
            raise ValueError('Setpoint must be below gate_ulim: ' + str(self.gate_ulim()) + 'V')
        elif setpoint < self.gate_llim():
            raise ValueError('Setpoint must be below gate_llim: ' + str(self.gate_llim()) + 'V')
        else:
            message = f"SOUR:{self._proper_function} " + f"{setpoint}"
            self.write_raw(message)
    ...
    def _get_source(self):
        message = f"SOUR:{self._proper_function}?"
        return self.ask(message)
    
    def _get_gate_ulim(self):
        return max(self.gate_limits)
    def _get_gate_llim(self):
        return min(self.gate_limits)
        
    def _set_gate_ulim(self, limit):
        if limit > self.gate_llim():
            self.gate_limits = (self.gate_llim(), limit)
        else:
            raise ValueError('Upper gate limit must be higher than lower gate limit: ' + str(self.gate_llim()))
    def _set_gate_llim(self, limit):
        if limit < self.gate_ulim():
            self.gate_limits = (limit, self.gate_ulim())
        else:
            raise ValueError('Lower gate limit must be lower than upper gate limit: ' + str(self.gate_ulim()))
